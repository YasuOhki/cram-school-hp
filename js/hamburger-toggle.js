...
$(document).ready(function() {
    $('.hamburger').on('click', function() {
        ...
        if (currentSrc.includes('hamburger-menu.svg')) {
            $icon.attr('src', 'image/icon/close.svg').attr('alt', 'メニューを閉じる');
            $button.addClass('hamburger--active');
        } 
        // close.svgが表示されている場合はhamburger-menu.svgに変更
        else if (currentSrc.includes('close.svg')) {
            $icon.attr('src', 'image/icon/hamburger-menu.svg').attr('alt', 'メニュー');
            $button.removeClass('hamburger--active');
        }
    });
});
